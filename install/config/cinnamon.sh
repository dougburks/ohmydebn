...
if [ ! -f $PANEL_STATE ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Configuring panel to be at top of screen"
...
if [ ! -f $PANEL_APPLET_STATE ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Configuring panel applets"
...
if [ ! -f $WINDOW_SPEED_STATE ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Setting Cinnamon window effect speed to maximum"
...
if [ ! -f $ALTTAB_STATE ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Configuring Cinnamon alttab switcher"
...
if [ ! -f $SPICE_STATE ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Configuring Cinnamon spices"
  for SPICE in "workspace-switcher@cinnamon.org" "notifications@cinnamon.org" "calendar@cinnamon.org"; do
    ...
    mkdir -p $SPICE_DIR
    /opt/ohmydebn/bin/ohmydebn-headline "cat" "Configuring Cinnamon spice $SPICE"
    cp -av /opt/ohmydebn/config/cinnamon/spices/$SPICE/* $SPICE_DIR
    echo
  done
  touch $SPICE_STATE
fi
...
if [ ! -f $GTILE_EXTENSION_STATE ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Installing gTile extension"
...
if [ ! -f $GTILE_CONFIG_STATE ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Configuring gTile extension"
  SPICE_DIR=~/.config/cinnamon/spices/gTile@OhMyDebn
  mkdir -p $SPICE_DIR
  cp -av /opt/ohmydebn/config/cinnamon/spices/gTile@OhMyDebn/* $SPICE_DIR
...
if [ ! -f $NEMO_CONFIG_STATE ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Configuring nemo file manager for list view"
