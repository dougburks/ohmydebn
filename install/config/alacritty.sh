...
if grep -q "alacritty/catppuccin-mocha.toml" $ALACRITTY_CONFIG >/dev/null 2>&1; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Removing old alacritty config"
...
if [ ! -d $ALACRITTY_DIR ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Configuring alacritty terminal emulator"
  mkdir -p ~/.config
  cp -av /opt/ohmydebn/config/alacritty ~/.config/
fi

# If this is the initial installation, then set alacritty as default terminal emulator
if [ ! -f ~/.local/state/ohmydebn ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Configuring alacritty as default terminal emulator"
