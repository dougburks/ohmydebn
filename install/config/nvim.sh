...
if [ ! -d $NVIM_CONFIG_DIR ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Configuring neovim with lazyvim"
  mkdir -p ~/.config
...
if grep -q "colorscheme = \"catppuccin\"" $NVIM_PLUGINS/core.lua >/dev/null 2>&1; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Disabling old static neovim theme config"
  mv $NVIM_PLUGINS/core.lua $NVIM_PLUGINS/core.lua.disabled
fi

NVIM_THEME=$NVIM_PLUGINS/theme.lua
if [ ! -L $NVIM_THEME ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Configuring neovim theme"
  ln -snf ~/.config/ohmydebn/current/theme/neovim.lua $NVIM_THEME
fi

LAZYVIM=$NVIM_CONFIG_DIR/lazyvim.json
if [ ! -f $LAZYVIM ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Creating $LAZYVIM"
  cp -av /opt/ohmydebn/config/nvim/lazyvim.json $LAZYVIM
fi

SCROLLING=$NVIM_CONFIG_DIR/lua/plugins/snacks-animated-scrolling-off.lua
if [ ! -f $SCROLLING ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Creating $SCROLLING"
  mkdir -p $NVIM_CONFIG_DIR/lua/plugins
  cp -av /opt/ohmydebn/config/nvim/lua/plugins/snacks-animated-scrolling-off.lua $SCROLLING
fi

TRANSPARENCY=$NVIM_CONFIG_DIR/plugin/after/transparency.lua
if [ ! -f $TRANSPARENCY ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Creating $TRANSPARENCY"
  mkdir -p $NVIM_CONFIG_DIR/plugin/after
  cp -av /opt/ohmydebn/config/nvim/plugin/after/transparency.lua $TRANSPARENCY
fi
...
if [ ! -f $CORE ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Creating $CORE"
  mkdir -p $NVIM_CONFIG_DIR/lua/plugins
  cp -av /opt/ohmydebn/config/nvim/lua/plugins/core.lua $CORE
...
if [ ! -f $ALL_THEMES ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Creating $ALL_THEMES"
  mkdir -p $NVIM_CONFIG_DIR/lua/plugins
  cp -av /opt/ohmydebn/config/nvim/lua/plugins/all-themes.lua $ALL_THEMES
fi

HOTRELOAD=$NVIM_CONFIG_DIR/lua/plugins/omarchy-theme-hotreload.lua
if [ ! -f $HOTRELOAD ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "cat" "Creating $HOTRELOAD"
  mkdir -p $NVIM_CONFIG_DIR/lua/plugins
  cp -av /opt/ohmydebn/config/nvim/lua/plugins/omarchy-theme-hotreload.lua $HOTRELOAD
fi
