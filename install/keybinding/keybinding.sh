...
KEYBINDING_STATE=$STATE_DIR/keybinding-20251115

if [ ! -f $KEYBINDING_STATE ]; then
  /opt/ohmydebn/bin/ohmydebn-headline "tte rain" "Updating hotkeys"

  KEYBINDING_DIR=/opt/ohmydebn/install/keybinding
  ...
  if pgrep -x cinnamon >/dev/null; then
    /opt/ohmydebn/bin/ohmydebn-headline "tte rain" "Restarting desktop to apply hotkey configuration"
    sleep 1s
