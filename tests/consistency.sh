...
# __pycache__ pruned: importing bin's underscored .py modules (or a test
# SourceFileLoader-ing any bin script) drops generated bytecode caches in
# bin/__pycache__/ - gitignored local artifacts, not source, and never
# executable.
done < <(find "$REPO_ROOT/bin" -type f -not -path "*/__pycache__/*"; find "$REPO_ROOT/install" -type f -name "*.sh")
...
# --exclude-dir=__pycache__ on both: generated bytecode caches carry the
# same marker strings as their source and would read as fake collisions.
mapfile -t MARKERS < <(grep -rhoE --exclude-dir=__pycache__ '~/\.local/state/ohmydebn-config/[a-zA-Z0-9_.-]+' \
  "$REPO_ROOT/install" "$REPO_ROOT/bin" 2>/dev/null | sort -u)
for marker in "${MARKERS[@]}"; do
  mapfile -t USING_FILES < <(grep -rl -F --exclude-dir=__pycache__ "$marker" "$REPO_ROOT/install" "$REPO_ROOT/bin" 2>/dev/null | sort -u)
...
mapfile -t ROFI_HITS < <(grep -rlE --exclude-dir=__pycache__ '/usr/bin/rofi\b' "$REPO_ROOT/bin" "$REPO_ROOT/install" 2>/dev/null)
...
echo
echo "-- Super+B launches the tiled browser wrapper (regression guard) --"
# Pins the Browser keybinding to ohmydebn-browser-tiled, not the bare
# ohmydebn-browser it wraps - an accidental rebind to the bare script
# (caught in review after happening for real) silently drops the
# full-screen-with-gaps auto-tiling for every browser, and no other
# check sees it: the grid/delegation checks above validate the wrapper
# itself, not who calls it, and the wrapper's own header claims the
# binding either way.
BROWSER_KB_COMMAND=$(grep '"Browser"' "$REPO_ROOT/install/keybinding/keybinding-custom.txt" | awk -F'"' '{print $4}')
if [[ "$BROWSER_KB_COMMAND" == "/usr/share/ohmydebn/bin/ohmydebn-browser-tiled" ]]; then
  echo "  \"Browser\" keybinding launches ohmydebn-browser-tiled"
else
  echo "  FAIL - \"Browser\" keybinding launches '$BROWSER_KB_COMMAND', not ohmydebn-browser-tiled"
  FAIL=$((FAIL + 1))
fi
...
for ENTRY in "${TITLE_OWNERS[@]}"; do
  LAUNCHER="${ENTRY%%:*}"
  TITLE="${ENTRY#*:}"
  # Three accepted spellings: the two shell forms, plus the Python
  # argv-list form ("--title", "Some Title") - ohmydebn-update-gui is a
  # Python GTK app now, but its no-VTE fallback path still launches the
  # old alacritty window this tile rule exists for.
  if ! grep -qF -- "--title '$TITLE'" "$REPO_ROOT/bin/$LAUNCHER" && ! grep -qF -- "--title \"$TITLE\"" "$REPO_ROOT/bin/$LAUNCHER" && ! grep -qF -- "\"--title\", \"$TITLE\"" "$REPO_ROOT/bin/$LAUNCHER"; then
...
echo
echo "-- ohmydebn-update-gui runtime prerequisites (regression guard) --"
# The update GUI embeds a VTE terminal and needs gir1.2-vte-2.91 at
# runtime; without it, it silently execs the old alacritty flow. That
# fallback is meant as a one-time transition (the very update it runs
# installs the dependency), not a permanent silent downgrade - so the
# dependency's presence in dependencies.sh is pinned here. The fallback
# also execs helper scripts by constructed path (os.path.join pieces, no
# single grep-able /usr/share reference), so a rename/removal of any of
# them - ohmydebn-update-pause is otherwise unreferenced and reads like
# dead code - would break the fallback without any other check noticing.
if grep -q 'gir1.2-vte-2.91' "$REPO_ROOT/install/packaging/dependencies.sh"; then
  echo "  gir1.2-vte-2.91 is in dependencies.sh"
else
  echo "  FAIL - bin/ohmydebn-update-gui needs gir1.2-vte-2.91 in install/packaging/dependencies.sh"
  FAIL=$((FAIL + 1))
fi
for HELPER in ohmydebn-update ohmydebn-update-pause ohmydebn-terminal; do
  if [[ -x "$REPO_ROOT/bin/$HELPER" ]]; then
    echo "  fallback helper bin/$HELPER exists and is executable"
  else
    echo "  FAIL - bin/ohmydebn-update-gui's fallback needs bin/$HELPER (missing or not executable)"
    FAIL=$((FAIL + 1))
  fi
done
